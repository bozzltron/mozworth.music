...
export default function MozworthAlbum() {
  ...
  return (
    <>
      ...
      <main class="min-h-screen min-w-full w-full flex items-center justify-center bg-gradient-to-br from-[#a04c3d] via-[#3e6e6b] to-[#e2c97b]">
        <article class="w-full h-full md:w-[98vw] md:h-[95vh] bg-black/45 md:rounded-xl shadow-2xl flex flex-col md:flex-row items-stretch p-4 md:mt-4 md:mb-4 overflow-hidden">
          {/* Left: Bandcamp embed, info, controls */}
          <div class="flex flex-col items-center md:items-start md:mr-10 flex-shrink-0 w-full md:w-[380px] max-w-full md:max-w-[380px] min-h-[420px] md:min-h-[470px] mb-6 md:mb-0">
            <div class="w-full max-w-[380px] min-h-[420px] md:min-h-[470px] h-[56vw] max-h-[380px] rounded-xl shadow-xl bg-[#222] overflow-hidden mb-6 md:mb-8">
              <iframe src="https://bandcamp.com/EmbeddedPlayer/album=2412424488/size=large/bgcol=ffffff/linkcol=0687f5/minimal=true/transparent=true/" class="w-full h-full min-h-[420px] md:min-h-[470px]" seamless title="mozworth by mozworth"></iframe>
            </div>
            ...
            <div class="player-controls flex flex-wrap gap-4 mb-4 w-full justify-center md:justify-start">
              <StreamingIcons links={streamingLinks} />
            </div>
          </div>
          ...
          <div class="player-right flex-1 flex flex-col min-w-0 md:max-h-[calc(95vh-2rem)] md:overflow-y-auto">
            {/* Tabs (desktop) */}
            <Show when={!isMobile()}>
              <div class="tabs flex gap-4 md:gap-8 mb-4 md:mb-6 mt-2 md:mt-4">
                {commentsEnabled() && (
                  <button class={`tab text-lg pb-1 border-b-2 transition-colors ${tab() === "conversation" ? "text-teal-400 border-teal-400" : "text-gray-400 border-transparent"}`} onClick={() => setTab("conversation")}>Conversation</button>
                )}
                <button class={`tab text-lg pb-1 border-b-2 transition-colors ${tab() === "tracks" ? "text-teal-400 border-teal-400" : "text-gray-400 border-transparent"}`} onClick={() => setTab("tracks")}>Tracklist</button>
                <button class={`tab text-lg pb-1 border-b-2 transition-colors ${tab() === "press" ? "text-teal-400 border-teal-400" : "text-gray-400 border-transparent"}`} onClick={() => setTab("press")}>Press Release</button>
                <button class={`tab text-lg pb-1 border-b-2 transition-colors ${tab() === "credits" ? "text-teal-400 border-teal-400" : "text-gray-400 border-transparent"}`} onClick={() => setTab("credits")}>Credits</button>
              </div>
              {commentsEnabled() && (
                <Show when={tab() === "conversation"}>
                  <section class="tab-content mb-8"><SongComments contentId="mozworth-album" /></section>
                </Show>
              )}
            ...
            {/* Mobile: show all tab content, with headings */}
            <Show when={isMobile()}>
              <div class="block sm:hidden w-full">
                {commentsEnabled() && (
                  <>
                    <div class="tab-heading text-lg font-bold mb-2 mt-4 text-teal-400">Conversation</div>
                    <section class="tab-content mb-8"><SongComments contentId="mozworth-album" /></section>
                  </>
                )}
                ...
                <div class="tab-heading text-lg font-bold mb-2 mt-4 text-teal-400">Credits</div>
                <section class="tab-content mb-8">
                  <p>Songwriting by Michael Bosworth</p>
                  <p>Composition by Michael Bosworth and Ken Mockler</p>
                  <p>Lyrics by Michael Bosworth</p>
